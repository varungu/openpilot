...
void QTouchButton::setDown(bool down) {
  qDebug() << "Set down:" << down;
...
MainWindow::MainWindow(QWidget *parent) : QWidget(parent) {
  ...
  for (int i = 0; i < 9; i++) {
    QTouchButton *btn = new QTouchButton(QString("Btn %1").arg(i));
    btn->setObjectName(QString("btn%1").arg(i));
//    QObject::connect(btn, &QTouchButton::clicked, this, [=]() { qDebug() << QString("Btn %1 clicked!").arg(i); });
